...
IMPORTANTE: Tomen de ejempo Facebook, ustedes pueden tener su base de datos con Usuarios, Publicaciones y Comentarios, sin embargo lo que importa más son las PUBLICACIONES y COMENTARIOS en este producto en particular, lo mismo que serían las de Instagram, Pinterest, etc. Lo de Registrar un Usuario sí cumple el criterio de un CREATE, pero el objetivo del proyecto es el de simular un producto REAL, por eso es que se valorará más si cumplen con lo básico de su producto, es decir 2 escenarios (o enunciados) por cada operación del CRUD, por importancia puede ser CREAR PUBLICACIÓN y CREAR COMENTARIO DENTRO DE UNA PUBLICACIÓN, o bien CREAR UN USUARIO y CREAR PUBLICACIÓN COMO USUARIO "JuanPerez123" (en este último caso se complementan AMBOS enunciados y se observa mayor INICIATIVA POR EL ESTUDIANTE/EQUIPO). 

Mientras que si sólo desarrollan 1 solo enunciado como "Registrar usuario" (Create), NO tiene tanto sentido ni tiene demasiada complejidad, mucho peor si sólo tienen 2 campos para dicho registro, y peor NO podrán realizar lo mencionado arriba, es decir, no podría demostrar un READ, como por ejemplo para "Listar publicaciones/mensajes/etc".
